...
function isProviderVisible(provider: Provider) {
  if (provider.name === "Metamask") return false;
  return true;
}
  ...
  <ul class="grid gap-[15px] px-2 pb-2">
    <li v-for="provider in providers" :key="provider.name">
      <button
        ...
        class="px-5 py-[15px] w-full dark:bg-slate-800 bg-slate-100 rounded-[40px] group transition-colors flex items-center gap-4"
        :class="
      ...
      >
        ...
        <svg
          ...
          :class="
            provider.name === 'Metamask' ? 'text-orange-500' : 'text-primary'
          "
