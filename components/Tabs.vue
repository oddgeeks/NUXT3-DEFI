  ...
  <div class="overflow-auto">
    <ul class="flex w-fit justify-center rounded-5 bg-slate-50 p-1.5 text-sm font-medium dark:bg-gray-850 sm:justify-normal sm:rounded-10">
      <li v-for="tab in tabs" :key="tab.label">
        <NuxtLink :class=" $route.query.tab === tab.query ? 'dark:bg-gray-900 bg-slate-150' : 'text-gray-400'" class="flex items-center gap-2.5 whitespace-nowrap rounded-2xl px-6 py-2.5 sm:rounded-7.5" :to="{ query: { tab: tab.query } }">
          {{ tab.label }}
