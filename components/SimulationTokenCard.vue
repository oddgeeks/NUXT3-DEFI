    ...
    <div class="flex justify-between w-full">
      ...
      >
        <span v-if="actualType === TypeTitles.In" class="leading-4 inline-flex items-center">
          From:&nbsp;{{ shortenHash(payload.from) }}&nbsp;<Copy type="button" class="w-3 h-3" :text="payload.from" :icon-only="true" /></span>
        <span v-else class="leading-4  inline-flex items-center"> To:&nbsp;{{ shortenHash(payload.to) }}&nbsp;<Copy type="button" class="w-3 h-3" :text="payload.to" :icon-only="true" /></span>
      </div>
      <div
        v-if="payload.type"
