<template>
  <div class="p-5 flex items-center border-b-[1px] last:border-b-[0px] dark:border-slate-750 border-white">
    <div class="relative basis-[30px] h-[30px] grow-0 shrink-0">
      <NFTImage :asset="asset" hide-chain-logo img-class="w-[30px] h-[30px] rounded-[6px]" />
      <ChainLogo
    ...
    <div class="ml-[10px] grow">
      <div class="text-sm dark:text-white text-slate-900 font-medium">{{ asset.name }}</div>
      <div v-if="asset.collectionName" class="text-xs text-slate-400 font-medium">{{ asset.collectionName }}</div>
    </div>
