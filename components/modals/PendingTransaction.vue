...
<template>
  <div class="text-center flex flex-col gap-7.5">
    ...
    <div v-if="!transaction" class="flex justify-center">
      <svg class="animate-spin -ml-1 mr-3 h-10 w-10 text-[#89BA4F]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <div>
      <h2 class="text-lg leading-5">Transaction {{ transaction ? (transaction.status ? "Confirmed" : "Failed") : "Pending" }}
    </h2>
    ...
    <p v-if="transaction && !transaction.status" class="text-slate-400 text-xs">Try again or return to the home page.</p>
     ...
     <div class="bg-slate-800 px-[18px] py-[14px] rounded-5 flex items-center justify-between">
