...
<template>
  ...
  <div v-if="!connection" class="space-y-8">
    <div class="inline-flex flex-col items-center">
      ...
      <p class="text-slate-400 text-xs text-center leading-5">You need the Avocado web app to be open to popup
        transactions.
        You
    ...
    <button class="bg-blue-500 hover:bg-blue-600 rounded-[15px] w-full h-12" @click="prepareAndConnect">Connect</button>
  </div>

  <div v-else class="space-y-8">
    <div class="flex flex-col items-center space-y-8">
      <div class="w-10 h-10" v-if="connection.peerMeta.icons.length">
        <img class="w-full h-full object-fit" :src="connection.peerMeta.icons[connection.peerMeta.icons.length - 1]" />
      </div>
