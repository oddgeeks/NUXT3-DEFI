...
<template>
  <div class="flex flex-col items-center space-y-4">
    ...
    <ul class="rounded-5 p-[6px] bg-slate-50 dark:bg-gray-850 w-full">
      <li class="flex items-center justify-between gap-2.5 pt-1 pb-3.5 px-3 rounded-[14px]">
        <span class="text-slate-400">Networks</span>
        <div
          @click="networkPreference = networkPreference.size === availableNetworks.length ? new Set() : new Set(availableNetworks.map((el: Network) => el.chainId))"
      ...
      <li
        ...
        :class="{ 'dark:text-slate-500 text-slate-400': !networkPreference.has(network.chainId) }"
        v-for="network in availableNetworks">
        <ChainLogo style="width: 30px; height: 30px" :chain="network.chainId" />
        {{ network.name }}
        <CheckCircle v-if="networkPreference.has(network.chainId)" class="success-circle cursor-pointer w-7 ml-auto" />
        <CheckCircle v-else class="svg-circle darker cursor-pointer w-7 ml-auto" />
      </li>
    </ul>
    <ClientOnly>
      <button :class="{
