...
<template>
  <div v-if="!safesLoading" class="flex items-center gap-2 sm:gap-4">
    ...
    <button v-if="allSafes?.length" class="flex h-[44px] w-full items-center justify-center gap-2.5 rounded-7.5 border  border-gray-800 bg-gray-850 py-1 pl-[14px] pr-2.5 text-left text-xs" @click="openAllWalletsModal()">
      All
