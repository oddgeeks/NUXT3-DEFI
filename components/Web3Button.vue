...
<template>
      ...
      <div class="relative dark:bg-gray-950 bg-white rounded-[30px] px-12 py-10 text-center">
        ...
        <ul class="grid gap-[15px] px-2 pb-2">
          <li :key="provider.name" v-for="provider in providers">
            <button v-if="isProviderVisible(provider)" @click="connect(closeModal, provider)"
              ...
              <svg v-if="loading[provider.name]" :class="
                provider.name === 'Metamask'
                  ? 'text-orange-500'
                  : 'text-blue-500'
              ...
              <svg v-else class="transition-all text-slate-500" :class="
                provider.name === 'Metamask'
  ...
  <div v-show="active" class="flex items-center gap-[14px]">
    <button class="px-4 py-[9px] flex items-center justify-between rounded-5 dark:bg-slate-800 bg-slate-100 gap-2"
      ...
      <span class="h-[26px] w-[26px] flex items-center justify-center bg-blue-500 rounded-full text-white">
        <PlusSVG />
      </span>
    </button>

    <span
      v-tippy="{ placement: 'bottom-end', content: trackingAccount ? `Tracking: ${shortenHash(account)}` : shortenHash(account) }">
